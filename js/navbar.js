...
layui.define(['element', 'common'], function(exports) {
	...
	Navbar.prototype.render = function() {
		...
		if(_config.data !== undefined && typeof(_config.data) === 'object') {//是否从本地加载
					...
					if($this.find('dl').length > 0) {
						var $dd = $this.find('dd').each(function() {
							$(this).on('click', function() {
								...
								var globalTabIdIndex= $a.attr('id');
								var data = {
									elem: $a,
									field: {
										href: href,
										icon: icon,
										title: title,
										globalTabIdIndex:globalTabIdIndex
									}
								}
								callback(data);
							});
	...
	function getHtml(data) {
		...
		for(var i = 0; i < data.length; i++) {
			if(data[i].spread) {//是否展开
				ulHtml += '<li class="layui-nav-item layui-nav-itemed">';
			} else {
				ulHtml += '<li class="layui-nav-item">';
			}
			if(data[i].children !== undefined && data[i].children.length > 0) {//一级标题
				...
				for(var j = 0; j < data[i].children.length; j++) {//遍历二级菜单
					ulHtml += '<dd>';
					ulHtml += '<a id="'+data[i].children[j].globalTabIdIndex+'" href="javascript:;" data-url="' + data[i].children[j].href + '">';
				...
				ulHtml += '<a  id="'+data[i].globalTabIdIndex+'" href="javascript:;" ' + dataUrl + '>';
				if(data[i].icon !== undefined && data[i].icon !== '') {
					if(data[i].icon.indexOf('fa-') !== -1) {
						ulHtml += '<i class="fa ' + data[i].icon + '" aria-hidden="true" data-icon="' + data[i].icon + '"></i>';
					} else {
