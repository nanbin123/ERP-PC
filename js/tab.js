...
layui.define(['element', 'common'], function(exports) {
	...
	Tab.prototype.tabAdd = function(data) {
		...
		if(tabIndex === -1) {
			var content = '<iframe src="' + data.href + '"lay-id="' + data.globalTabIdIndex + '"></iframe>';
			var title = '';
			...
			if(_config.closed) {
				title += '<i class="layui-icon layui-unselect layui-tab-close" lay-id="' + data.globalTabIdIndex + '">&#x1006;</i>';
			}
			//添加tab
			element.tabAdd(ELEM.tabFilter, {
				title: title,
				content: content,
				id:data.globalTabIdIndex
			});
			//iframe 自适应
			ELEM.contentBox.find('iframe[lay-id=' + data.globalTabIdIndex + ']').each(function() {
				$(this).height(ELEM.contentBox.height());
			});
			if(_config.closed) {
				//监听关闭事件
				ELEM.titleBox.find('li').children('i.layui-tab-close[lay-id=' + data.globalTabIdIndex + ']').on('click', function() {
					element.tabDelete(ELEM.tabFilter, $(this).parent('li').attr('lay-id')).init();
				});
			};
			//切换到当前打开的选项卡
			element.tabChange(ELEM.tabFilter,data.globalTabIdIndex);
		} else {
			element.tabChange(ELEM.tabFilter, tabIndex);
			//自动刷新
			if(_config.autoRefresh) {
				ELEM.contentBox.find('iframe[lay-id=' + tabIndex + ']')[0].contentWindow.location.reload();
			}
		}
	};
