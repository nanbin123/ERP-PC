...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function(exports){
  ...
  //字符常量
  ,MOD_NAME = 'table2', ELEM = '.layui-table', THIS = 'layui-this', SHOW = 'layui-show', HIDE = 'layui-hide', DISABLED = 'layui-disabled', NONE = 'layui-none'
  ...
  Class.prototype.renderData = function(res, curr, count, sort){
    var that = this
        ...
        ,render = function(){ //后续性能提升的重点
          ...
          //layui.each(data, function(i1, item1){//123
          for(var i1 = 0; i1 < addnumber; i1++){
            var item1 = data[i1];
            ...
            //判断是否有下级的tr
            if(item1.recordList.length != 0){
              layui.each(item1.recordList, function(i2, item2){
                //下级行
                var td2 = [], tds_fixed2 = [], tds_fixed_r2 = []
                    ,numbers2 = i2 + options.limit*(curr - 1) + 1; //序号

                that.eachCols(function(i4, item4){
                  var field = item4.field || i4
                      ,key = options.index + '-' + item4.key
                      ,content = item2[field];

                  if(content === undefined || content === null) content = '';
                  if(item4.colGroup) return;

                  //td内容
                  var tdb = ['<td style="font-size: 13px" data-field="'+ field +'" data-key="'+ key +'" '+ function(){ //追加各种属性
                    var attr = [];
                    if(item4.templet) attr.push('data-content="'+ content +'"'); //自定义模板
                    if(item4.align) attr.push('align="'+ item4.align +'"'); //对齐方式
                    if(item4.event) attr.push('lay-event="'+ item4.event +'"'); //自定义事件
                    return attr.join(' ');
                  }() +' class="'+ function(){ //追加样式
                    var classNames = [];
                    if(item4.hide) classNames.push(HIDE); //插入隐藏列样式
                    if(!item4.field) classNames.push('layui-table-col-special'); //插入特殊列样式
                    return classNames.join(' ');
                  }() +'">'
                    ,'<div class="layui-table-cell laytable-cell-'+ function(){ //返回对应的CSS类标识
                      return item4.type === 'normal' ? key
                          : (key + ' laytable-cell-' + item4.type);
                    }() +'">' + function(){
                      var tplData = $.extend(true, {
                        LAY_INDEX: numbers2
                      }, item2)
                  ...
                  if(item4.fixed && item4.fixed !== 'right'){
                    tds_fixed2.push(tdb);
                  }
                  if(item4.fixed === 'right'){
                    tds_fixed_r2.push(tdb);
                  }
                });

                trs.push('<tr data-index="'+ data.length +'" style="background-color:#fff;color: #7B7DE5;">'+ td2.join('') + '</tr>');
                trs_fixed.push('<tr  data-index="'+ data.length +'">'+ tds_fixed2.join('') + '</tr>');
                trs_fixed_r.push('<tr  data-index="'+ data.length +'">'+ tds_fixed_r2.join('') + '</tr>');
                data.push(item2);
               /* data.length--;
                addnumber++;*/
              })
